<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="日记名称" prop="rijimingcheng">
          <el-input v-model="ruleForm.rijimingcheng" 
              placeholder="日记名称" clearable  :readonly="ro.rijimingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="日记名称" prop="rijimingcheng">
              <el-input v-model="ruleForm.rijimingcheng" 
                placeholder="日记名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.rijitupian" label="日记图片" prop="rijitupian">
          <file-upload
          tip="点击上传日记图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.rijitupian?ruleForm.rijitupian:''"
          @change="rijitupianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.rijitupian" label="日记图片" prop="rijitupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.rijitupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="日记便签" prop="rijibianqian">
          <el-input v-model="ruleForm.rijibianqian" 
              placeholder="日记便签" clearable  :readonly="ro.rijibianqian"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="日记便签" prop="rijibianqian">
              <el-input v-model="ruleForm.rijibianqian" 
                placeholder="日记便签" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="日记日期" prop="rijiriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.rijiriqi" 
                type="date"
                :readonly="ro.rijiriqi"
                placeholder="日记日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.rijiriqi" label="日记日期" prop="rijiriqi">
              <el-input v-model="ruleForm.rijiriqi" 
                placeholder="日记日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="日记内容" prop="rijineirong">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="日记内容"
                  v-model="ruleForm.rijineirong" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.rijineirong" label="日记内容" prop="rijineirong">
                    <span>{{ruleForm.rijineirong}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	yonghuming : false,
      ...
      rules: {
          rijimingcheng: [
                { required: true, message: '日记名称不能为空', trigger: 'blur' },
          ],
          rijitupian: [
          ],
          rijibianqian: [
                { required: true, message: '日记便签不能为空', trigger: 'blur' },
          ],
          rijineirong: [
          ],
          rijiriqi: [
  ...
  created() {
	this.ruleForm.rijiriqi = this.getCurDate()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.rijimingcheng = true;
          ...
          if(o=='rijitupian'){
            this.ruleForm.rijitupian = obj[o];
	    this.ro.rijitupian = true;
            continue;
          }
          if(o=='rijibianqian'){
            this.ruleForm.rijibianqian = obj[o];
	    this.ro.rijibianqian = true;
            continue;
          }
          if(o=='rijineirong'){
	    ...
	    this.ro.rijineirong = true;
          ...
          if(o=='rijiriqi'){
            this.ruleForm.rijiriqi = obj[o];
	    this.ro.rijiriqi = true;
            continue;
          }
          if(o=='yonghuming'){
	...
	if(this.ruleForm.rijitupian!=null) {
		this.ruleForm.rijitupian = this.ruleForm.rijitupian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yonghuriji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yonghurijiCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    rijitupianUploadChange(fileUrls) {
	this.ruleForm.rijitupian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
